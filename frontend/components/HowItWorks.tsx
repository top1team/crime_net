...
const HowItWorks: React.FC = () => {
    ...
    const ReportCard = ({ data, onClick, isFaded }: { data: any, onClick: () => void, isFaded: boolean }) => (
        <div 
            onClick={onClick}
            className={`bg-stone-50 border border-stone-200 rounded-2xl p-8 flex flex-col h-full cursor-pointer transition-all duration-500 ease-in-out shadow-md ${isFaded ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'} ${data.feature.hover} hover:shadow-xl`}
        >
             <div className="flex items-center gap-4">
                {data.icon}
                <h3 className="text-2xl font-bold text-stone-800">{data.title}</h3>
            </div>
            <ul className="mt-6 space-y-4 text-stone-600 list-disc list-inside">
    ...
    const DetailPanel = ({ data, onClose, isVisible, arrowDirection }: { data: any, onClose: () => void, isVisible: boolean, arrowDirection: 'left' | 'right' }) => (
        <div className={`bg-stone-50 border rounded-2xl p-8 flex flex-col h-full absolute top-0 left-0 w-full transition-all duration-500 ease-in-out ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'} ${data.detail.style}`}>
            <div className="flex items-center gap-4 mb-8">
                {data.icon}
                <h3 className="text-2xl font-bold text-stone-800">{data.detail.title}</h3>
            </div>
            <div className="space-y-6 flex-grow">
                {data.detail.steps.map((step: {title: string, description: string}, index: number) => (
                    <div key={index} className="flex items-start gap-4">
                        <div className="flex-shrink-0 w-8 h-8 bg-stone-100 border border-stone-200 rounded-full flex items-center justify-center font-bold text-stone-700 mt-1">
                            <span>{index + 1}</span>
                        </div>
                        <div>
                            <h4 className="text-lg font-semibold text-stone-800">{step.title}</h4>
            ...
            <button
                onClick={onClose}
                className={`mt-8 inline-flex items-center gap-2 text-sm font-bold text-stone-600 hover:text-stone-800 transition-colors group ${arrowDirection === 'left' ? 'self-start' : 'self-end'}`}
    ...
    return (
        <section className="bg-stone-50 py-20 sm:py-24 border-t border-stone-200">
